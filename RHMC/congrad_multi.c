...
int congrad_multi(vector *src, vector **psim,
  ...
  double rsqj;
  vector *mpm = malloc(sites_on_node * sizeof(*mpm));
  vector *pm0 = malloc(sites_on_node * sizeof(*pm0));
  vector *rm  = malloc(sites_on_node * sizeof(*rm));
  vector **pm = malloc(Norder * sizeof(**pm));
  for (i = 1; i < Norder; i++)    // !!!
    pm[i] = malloc(sites_on_node * sizeof(vector));
